...
export default function StockTradeList({ trades, onEdit, onDelete }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    return trades
      ...
      .filter((t) => !search || t.ticker.toLowerCase().includes(normalizedSearch) || t.note.toLowerCase().includes(normalizedSearch))
      ...
      <div className="space-y-3 tab-content">
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          <div className="bg-[#1C1C1E] rounded-2xl px-3 py-3.5">
            ...
            <p className={`text-[13px] font-extrabold num leading-tight ${totalRealizedPnL > 0 ? 'text-[#2ACF6A]' : totalRealizedPnL < 0 ? 'text-[#F25260]' : 'text-[#8B95A1]'
              }`}>
        ...
        <div className="space-y-2">
          <div className="flex gap-1 bg-[#1C1C1E] p-1 rounded-xl">
            {(['all', 'buy', 'sell'] as Filter[]).map((f) => (
              <button key={f}
                onClick={() => setFilter(f)}
                className={`flex-1 py-1.5 rounded-lg text-xs font-bold transition-colors ${filter === f ? 'bg-[#2C2C2E] text-white' : 'text-[#4E5968]'
                  }`}>
                    ...
                    return (
                      <div key={t.id} className="group flex items-center gap-3 px-4 py-3">
                        <button
                          ...
                          className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 active:opacity-70 transition-opacity ${isBuy ? 'bg-[#3D8EF8]/15' : 'bg-[#F25260]/15'
                            }`}
                        >
                          {isBuy
