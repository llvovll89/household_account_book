...
const AutoApplyRecurringModal = lazy(
    () => import("./components/AutoApplyRecurringModal"),
);
const AutoApplyResultModal = lazy(
    () => import("./components/AutoApplyResultModal"),
);
const SyncConflictModal = lazy(() => import("./components/SyncConflictModal"));
const SyncRecoveryGuideModal = lazy(
    () => import("./components/SyncRecoveryGuideModal"),
);
const AutoCategoryRuleModal = lazy(
    () => import("./components/AutoCategoryRuleModal"),
);
...
// 신규 유저 기본 숨김 위젯 (활성화 빈도 낮은 항목)
const DEFAULT_HIDDEN_WIDGETS: DashboardWidgetId[] = [
    "timeofday-spending",
    "weekly-comparison",
    "spending-pace",
    "spending-spike",
...
function parseConflictScopePreference(): RemoteVersionKey[] {
    try {
        ...
        const allowed: RemoteVersionKey[] = [
            "transactions",
            "memos",
            "budgets",
            "recurring",
        ...
        const result = parsed.filter((item): item is RemoteVersionKey =>
            allowed.includes(item as RemoteVersionKey),
        );
...
function getDefaultSelectedConflictKeys(
    keys: RemoteVersionKey[],
    preferredScopes: RemoteVersionKey[],
): RemoteVersionKey[] {
...
function getRetryReasonTotals(
    reasonsByScope: Partial<Record<RemoteVersionKey, RetryReasonCode[]>>,
) {
    let conflict = 0;
    let saveFailed = 0;
...
type ConflictVersionDiff = Partial<
    Record<RemoteVersionKey, {expected: number; remote: number}>
>;
type ConflictCountDiff = Partial<
    Record<
        RemoteVersionKey,
        {localCount: number | null; remoteCount: number | null}
    >
>;
    ...
    switch (action.type) {
        case "OPEN_TX_MODAL":
            return {
                ...state,
                showModal: true,
                editingTransaction: action.editing ?? null,
            };
        case "CLOSE_TX_MODAL":
        ...
        case "TRIGGER_SUB":
            return {
                ...state,
                subscriptionAddTrigger: state.subscriptionAddTrigger + 1,
            };
        case "TRIGGER_GOAL":
            return {...state, goalAddTrigger: state.goalAddTrigger + 1};
        case "OPEN_CONFIRM":
            return {
                ...state,
                confirmModal: {
...
function GoogleIcon() {
    return (
        <svg width="16" height="16" viewBox="0 0 48 48" aria-hidden="true">
            <path
                fill="#FFC107"
                d="M43.6 20.5H42V20H24v8h11.3C33.6 32.7 29.2 36 24 36c-6.6 0-12-5.4-12-12s5.4-12 12-12c3 0 5.7 1.1 7.8 3l5.7-5.7C34.1 6.1 29.3 4 24 4C12.9 4 4 12.9 4 24s8.9 20 20 20 20-8.9 20-20c0-1.3-.1-2.4-.4-3.5z"
            />
            <path
                fill="#FF3D00"
            ...
            <path
                fill="#4CAF50"
                d="M24 44c5.2 0 10-2 13.5-5.2l-6.2-5.2C29.4 35 26.9 36 24 36c-5.2 0-9.6-3.3-11.2-8l-6.5 5C9.6 39.5 16.3 44 24 44z"
            />
            <path
                fill="#1976D2"
                d="M43.6 20.5H42V20H24v8h11.3c-1 2.9-3 5.2-5.9 6.8l6.2 5.2C39.3 36.6 44 31 44 24c0-1.3-.1-2.4-.4-3.5z"
            />
        </svg>
    );
}
...
export default function App() {
    ...
    const [autoCategoryRules, setAutoCategoryRules] = useState<
        AutoCategoryRule[]
    >([]);
    ...
    const [showAutoCategoryRuleModal, setShowAutoCategoryRuleModal] =
        useState(false);
    ...
    const [failedPersistTasks, setFailedPersistTasks] = useState<
        FailedPersistTask[]
    >([]);
    const [lastRetryReasons, setLastRetryReasons] = useState<
        Partial<Record<RemoteVersionKey, RetryReasonCode[]>>
    >({});
    ...
    const [lastRetryResult, setLastRetryResult] =
        useState<RetryResultSummary | null>(null);
    ...
    const [showSyncRecoveryGuideModal, setShowSyncRecoveryGuideModal] =
        useState(false);
    const [conflictKeys, setConflictKeys] = useState<RemoteVersionKey[]>([]);
    const [selectedConflictKeys, setSelectedConflictKeys] = useState<
        RemoteVersionKey[]
    >([]);
    const [conflictVersionDiffs, setConflictVersionDiffs] =
        useState<ConflictVersionDiff>({});
    const [conflictCountDiffs, setConflictCountDiffs] =
        useState<ConflictCountDiff>({});
    const [conflictRemoteUpdatedAt, setConflictRemoteUpdatedAt] = useState<
        number | null
    >(null);
    const [conflictPreferredScopes, setConflictPreferredScopes] = useState<
        RemoteVersionKey[]
    >(() => parseConflictScopePreference());
    const [rememberConflictPolicy, setRememberConflictPolicy] =
        useState<boolean>(() => parseConflictPolicyRemember());
    ...
    const [autoApplyResultItems, setAutoApplyResultItems] = useState<
        RecurringTransaction[]
    >([]);
    const [showAutoApplyResultModal, setShowAutoApplyResultModal] =
        useState(false);
    const [autoApplyMode, setAutoApplyMode] = useState<AutoApplyRecurringMode>(
        () => parseAutoApplyRecurringMode(),
    );
    ...
    useEffect(() => {
        return registerToastHandler(
            (msg, duration = 2500, variant = "success", action) => {
                setToastMsg(msg);
    ...
    useEffect(() => {
        const onOnline = () => {
            ...
            showToast(
                "온라인으로 전환됐어요. 미동기화 항목을 다시 저장할 수 있어요.",
            );
        };
        const onOffline = () => {
            setIsOnline(false);
            showToast(
                "오프라인 상태예요. 변경사항은 임시 저장 후 연결 복구 시 동기화돼요.",
            );
        };
    ...
    useEffect(() => {
        const onSettingsUpdated = () => {
            void loadSettings().then((settings) => {
                ...
                setHiddenWidgets(
                    settings.hiddenWidgets ?? DEFAULT_HIDDEN_WIDGETS,
                );
            });
    ...
    useEffect(() => {
        localStorage.setItem(
            CONFLICT_SCOPE_PREF_KEY,
            JSON.stringify(conflictPreferredScopes),
        );
    }, [conflictPreferredScopes]);

    useEffect(() => {
        localStorage.setItem(
            CONFLICT_POLICY_REMEMBER_KEY,
            rememberConflictPolicy ? "1" : "0",
        );
    ...
    useEffect(() => {
        ...
        localStorage.setItem(
            LAST_SYNC_SUCCESS_AT_KEY,
            String(lastSyncSuccessAt),
        );
    }, [lastSyncSuccessAt]);
    ...
    const hydrateData = useCallback(async () => {
        ...
        setHiddenWidgets(
            snapshot.settings.hiddenWidgets ?? DEFAULT_HIDDEN_WIDGETS,
        );
    }, []);
    ...
    const handleLedgerTabChange = useCallback(
        (nextTab: Tab) => {
    ...
    useEffect(() => {
        ...
        if (transitionStartRef.current === null || !transitionLabelRef.current)
            return;
        ...
        const frame1 = requestAnimationFrame(() => {
            const frame2 = requestAnimationFrame(() => {
                ...
                console.debug(
                    `[tab-transition] ${label} ${elapsed.toFixed(1)}ms`,
                );
            });
            void frame2;
        });
                ...
                .catch((e) => {
                    ...
                    if (isStorageConflictError(e)) {
                        ...
                        setSelectedConflictKeys(
                            getDefaultSelectedConflictKeys(
                                keys,
                                conflictPreferredScopes,
                            ),
                        );
    ...
    const retryFailedPersistTasks = useCallback(
        (allowedScopes?: RemoteVersionKey[], mode: RetryRunMode = "all") => {
            if (isRetryingPersist) return;
            const allow =
                allowedScopes && allowedScopes.length > 0
                    ? new Set<RemoteVersionKey>(allowedScopes)
                    : null;
            const queued = allow
                ...
                try {
                    ...
                    for (let i = 0; i < queued.length; i++) {
                        ...
                        try {
                            ...
                            const reason: "conflict" | "save-failed" =
                                isStorageConflictError(e)
                                    ? "conflict"
                                    : "save-failed";
                            if (isStorageConflictError(e)) {
                                const keys = Array.from(
                                    new Set(e.conflictKeys),
                                );
                                setConflictKeys(keys);
                                setSelectedConflictKeys(
                                    getDefaultSelectedConflictKeys(
                            ...
                            const reasons =
                                failedReasons.get(item.scope) ??
                                new Set<"conflict" | "save-failed">();
                            reasons.add(reason);
                    ...
                    const nextReasonState: Partial<
                        Record<RemoteVersionKey, RetryReasonCode[]>
                    > = {};
                    ...
                    if (stillFailed.length === 0) {
                        ...
                        setLastSyncSuccessAt(Date.now());
                        const successLabels =
                            Array.from(succeededScopes).map(scopeLabel);
                        const summary =
                    ...
                    setHasPendingSync(true);
                    const failedLabels =
                        Array.from(failedScopes).map(scopeLabel);
                    const failedSummary =
                    ...
                    if (conflictCount > 0)
                        reasonParts.push(`충돌 ${conflictCount}범위`);
                    if (saveFailedCount > 0)
                        reasonParts.push(`저장오류 ${saveFailedCount}범위`);
                    const reasonSuffix =
                        reasonParts.length > 0
                            ? ` (${reasonParts.join(" · ")})`
                            : "";
                    showToast(
                        `재시도 후 ${stillFailed.length}건이 남았어요.${failedSummary}${reasonSuffix}`,
                    );
                } finally {
    ...
    const getScopesByReasons = useCallback(
        (targets: RetryReasonCode[]): RemoteVersionKey[] => {
            const targetSet = new Set<RetryReasonCode>(targets);
            return Object.entries(lastRetryReasons)
                .filter(([, codes]) =>
                    (codes ?? []).some((code) => targetSet.has(code)),
                )
    ...
    const handleRetryMineAfterSync = useCallback(() => {
        if (rememberConflictPolicy) {
            setConflictPreferredScopes((prev) => {
                ...
                const selected = new Set<RemoteVersionKey>(
    ...
    useEffect(() => {
        ...
        if (autoApplyMode === "always") {
            ...
            void handleApplyRecurring(pending, todayYM)
                .then(() => {
                    showToast(
                        `정기 항목 ${pending.length}건 자동 등록: ${formatRecurringToastSummary(pending)}`,
                    );
    ...
    const openTransactionsWithBilling = useCallback(
        (billing: "current" | "next") => {
    ...
    const failedScopeDetails = useMemo(() => {
        const map = new Map<
            RemoteVersionKey,
            {count: number; maxAttempts: number; reasons: RetryReasonCode[]}
        >();
    ...
    const conflictRetryScopes = useMemo(
        () => getScopesByReasons(["conflict"]),
        [getScopesByReasons],
    );
    const saveFailedRetryScopes = useMemo(
        () => getScopesByReasons(["save-failed"]),
        [getScopesByReasons],
    );
    const retryRatio =
        retryProgress && retryProgress.total > 0
            ...
            : 0;
    const showSyncStatusBar =
        !isOnline || hasPendingSync || pendingCount > 0 || isRetryingPersist;
    const syncStatusBarMeta = useMemo(() => {
        ...
        if (!isOnline) {
            return {
                title: hasPendingSync
                    ? `오프라인 · 저장 대기 ${pendingCount}건`
                    : "오프라인 상태",
                detail: hasPendingSync
                    ? "연결 복구 시 자동 재시도할 수 있어요"
                    : "변경사항은 로컬에 임시 저장됩니다",
        ...
        pendingCount,
        retryProgress,
        retryReasonTotals.conflict,
        retryReasonTotals.saveFailed,
    ]);
        ...
        return (
            <div className="min-h-screen bg-[#111111] flex items-center justify-center px-6">
                <div className="w-full max-w-sm rounded-3xl bg-[#1C1C1E] border border-[rgba(255,255,255,0.08)] shadow-2xl shadow-black/40 p-6 text-center">
                    ...
                    <h2 className="mt-4 text-[17px] font-extrabold text-white tracking-tight">
                        잔고플랜 준비 중
                    </h2>
                    <p className="mt-1 text-sm text-[#8B95A1] font-medium">
                        데이터를 안전하게 불러오고 있어요
                    </p>
        ...
        <div className="min-h-screen bg-[#111111] pb-nav-safe">
            {needRefresh && (
                <div className="fixed bottom-toast-safe left-1/2 -translate-x-1/2 z-50 w-[calc(100%-2.5rem)] max-w-sm">
                    <div className="flex items-center gap-3 bg-[#1C1C1E] border border-[rgba(255,255,255,0.08)] rounded-2xl px-4 py-3.5 shadow-xl">
                        <RefreshCw
                            size={16}
                            className="text-[#3D8EF8] shrink-0 animate-spin"
            ...
            <header className="bg-[#111111] sticky top-0 z-40">
                <div className="max-w-lg mx-auto px-5 pt-header-safe pb-3">
                                    ...
                                    {showUserMenu && (
                                        <div className="absolute right-0 top-[calc(100%+8px)] z-50 w-48 rounded-2xl bg-[#1C1C1E] border border-[rgba(255,255,255,0.08)] shadow-2xl shadow-black/40 overflow-hidden">
                                            <div className="px-3.5 py-3 border-b border-[rgba(255,255,255,0.06)]">
                                                <p className="text-[10px] font-semibold text-[#4E5968] uppercase tracking-wide mb-0.5">
                                                    로그인 계정
                                                </p>
                                                <p className="text-[12px] font-semibold text-[#C8D1DC] truncate">
                                                    {user.email ??
                                                        "로그인 사용자"}
                                                </p>
                                            </div>
                            ...
                            {(monthIncome > 0 || monthExpense > 0) && (
                                <div className="mt-3 pb-1 space-y-1.5">
                                    <div className="flex items-center justify-center gap-4">
                                        <span className="text-xs font-semibold text-[#2ACF6A] num">
                                            +{monthIncome.toLocaleString()}
                                        </span>
                                        <div className="w-1 h-1 rounded-full bg-[rgba(255,255,255,0.12)]" />
                                        <span className="text-xs font-semibold text-[#F25260] num">
                                            -{monthExpense.toLocaleString()}
                                        </span>
                                        <div className="w-1 h-1 rounded-full bg-[rgba(255,255,255,0.12)]" />
                                        <span
                                            className={`text-xs font-bold num ${monthBalance >= 0 ? "text-white" : "text-[#F25260]"}`}
                                        >
                                    ...
                                    {(monthCardDue > 0 ||
                                        ...
                                        <div className="flex items-center justify-center gap-2 flex-wrap">
                                            ...
                                            >
                                                다음청구{" "}
                                                {nextMonthCardDue.toLocaleString()}
                                            </button>
                                        </div>
                                    )}
                                </div>
                    ...
                    {showSyncStatusBar && (
                        <div
                            className={`mt-3 rounded-2xl border px-3 py-2.5 flex items-center gap-3 ${syncStatusBarMeta.toneClass}`}
                        >
                            {!isOnline ? (
                                <WifiOff
                                    size={14}
                                    className={`${syncStatusBarMeta.iconClass} shrink-0`}
                                />
                            ) : isRetryingPersist ? (
                                <RefreshCw
                                    size={14}
                                    className={`${syncStatusBarMeta.iconClass} shrink-0 animate-spin`}
                                />
                            ) : (
                                <CloudOff
                                    size={14}
                            ...
                            <div className="min-w-0 flex-1">
                                <p className="text-[11px] font-bold text-[#E8EDF5]">
                                    {syncStatusBarMeta.title}
                                </p>
                                <p className="text-[10px] text-[#B6C0CE] truncate">
                                    {syncStatusBarMeta.detail}
                                </p>
                                {lastSyncSuccessAt && (
                                    <p
                                        ...
                                        <div className="mt-1.5 grid grid-cols-1 gap-1">
                                            {failedScopeDetails
                                                ...
                                                .map((item) => (
                                                    <div
                                                        key={item.scope}
                                                        className="flex items-center justify-between px-2 py-1 rounded-lg bg-[#111827]/45 border border-white/5"
                                                    >
                                                        <div className="min-w-0">
                                                            <span className="text-[10px] text-[#C8D1DC] font-semibold">
                                                                {scopeLabel(
                                                                    item.scope,
                                                                )}
                                                            </span>
                                                            {item.reasons
                                                                ...
                                                                <div className="flex items-center gap-1 mt-0.5">
                                                                    {item.reasons.includes(
                                                                        ...
                                                                        <span className="text-[9px] px-1 py-0.5 rounded bg-[#F25260]/15 text-[#FFD5D9]">
                                                                            충돌
                                                                        </span>
                                                                    )}
                                                                    {item.reasons.includes(
                                                                        ...
                                                                        <span className="text-[9px] px-1 py-0.5 rounded bg-[#3D8EF8]/15 text-[#CFE2FF]">
                                                                            저장오류
                                                                        </span>
                                                                    )}
                                                                </div>
                                                        ...
                                                        <span className="text-[10px] text-[#8B95A1] num">
                                                            {item.count}건 ·
                                                            재시도{" "}
                                                            {item.maxAttempts}회
                                                        </span>
                            ...
                            <div className="flex flex-col items-end gap-1.5">
                                <div className="flex items-center gap-1.5">
                                    ...
                                    {Object.keys(lastRetryReasons).length >
                                        0 && (
                                        <button
                                            onClick={() =>
                                                setShowSyncRecoveryGuideModal(
                                                    true,
                                                )
                                            }
                                            className="px-2 py-1.5 rounded-lg bg-[#2C2C2E] text-[#C8D1DC] text-[11px] font-bold border border-white/10"
                ...
                {activeMode === "ledger" && (
                    <Suspense fallback={<WorkspaceSkeleton mode="ledger" />}>
                        <LedgerWorkspace
                            ...
                            onOpenCategoryModal={() =>
                                dispatchUI({type: "SET_CATEGORY", value: true})
                            }
                            ...
                            onTransactionEdit={(t) =>
                                dispatchUI({type: "OPEN_TX_MODAL", editing: t})
                            }
                                ...
                                <>
                                    ...
                                    {fabExpanded && isSpeedDialTab && (
                                        <button
                                            ...
                                            style={{
                                                ...
                                                boxShadow:
                                                    "0 4px 16px rgba(26,140,78,0.45)",
                                            }}
                                            onClick={() => {
                                    ...
                                    {fabExpanded && isSpeedDialTab && (
                                        <button
                                            ...
                                            style={{
                                                backgroundColor: "#C0394A",
                                                animationDelay: "0s",
                                                boxShadow:
                                                    "0 4px 16px rgba(192,57,74,0.45)",
                                            }}
                                            onClick={() => {
                                                setTxInitialType("expense");
                                                setFabExpanded(false);
                ...
                <div className="fixed left-1/2 -translate-x-1/2 bottom-banner-safe z-50 w-[calc(100%-2.5rem)] max-w-sm">
                    <div className="bg-[#1C1C1E] border border-[rgba(255,255,255,0.08)] rounded-2xl px-4 py-3.5 shadow-xl">
                        ...
                        <p className="text-[11px] text-[#8B95A1] mt-1">
                            {isIosManualInstall
                                ? installGuideText
                                : "설치 버튼을 눌러 잔고플랜 앱을 설치할 수 있어요."}
                        </p>
                        <div className="mt-3 flex items-center justify-end gap-2">
                            <button
                                onClick={closeInstallBanner}
                                className="px-3 py-1.5 rounded-xl bg-[#2C2C2E] text-[#8B95A1] text-xs font-bold"
                            >
                                닫기
                            </button>
            ...
            {toastMsg && (
                <div
                    className={`fixed bottom-toast-safe left-1/2 -translate-x-1/2 z-50 w-[calc(100%-2.5rem)] max-w-sm ${toastAction ? "pointer-events-auto" : "pointer-events-none"}`}
                >
                    <div className="flex items-center gap-3 bg-[#1C1C1E] border border-[rgba(255,255,255,0.08)] rounded-2xl px-4 py-3 shadow-xl">
                        {toastVariant === "success" && (
                            <CheckCircle2
                                size={16}
                                className="text-[#2ACF6A] shrink-0"
                            />
                        )}
                        {toastVariant === "warning" && (
                            <AlertTriangle
                                size={16}
                                className="text-[#F5BE3A] shrink-0"
                            />
                        )}
                        {toastVariant === "error" && (
                            <AlertTriangle
                                size={16}
                                className="text-[#F25260] shrink-0"
                            />
                        )}
                        {toastVariant === "info" && (
                            <Info
                                size={16}
                                className="text-[#3D8EF8] shrink-0"
                            />
                        )}
                        <p className="text-sm font-semibold text-white whitespace-pre-line flex-1">
                            {toastMsg}
                        </p>
                        {toastAction && (
                            <button
            ...
            {showHelp && (
                <Suspense fallback={null}>
                    <HelpModal
                        onClose={() =>
                            dispatchUI({type: "SET_HELP", value: false})
                        }
                    />
                </Suspense>
            )}
            ...
            {showModal && (
                <Suspense fallback={null}>
                    <TransactionModal
                        ...
                        initialType={
                            editingTransaction ? undefined : txInitialType
                        }
                    />
                </Suspense>
            )}
            {showCategoryModal && (
                <Suspense fallback={null}>
                    <CategoryModal
                        ...
                        onClose={() =>
                            dispatchUI({type: "SET_CATEGORY", value: false})
                        }
                    />
                </Suspense>
            )}
            ...
            {showSyncConflictModal && (
                <Suspense fallback={null}>
                    <SyncConflictModal
                        ...
                        recommendedKeys={getDefaultSelectedConflictKeys(
                            conflictKeys,
                            conflictPreferredScopes,
                        )}
                        ...
                        onToggleKey={(key) =>
                            toggleConflictKey(key as RemoteVersionKey)
                        }
            ...
            {showSyncRecoveryGuideModal && (
                <Suspense fallback={null}>
                    <SyncRecoveryGuideModal
                        reasons={Array.from(
                            new Set(
                                Object.values(lastRetryReasons).flatMap(
                                    (codes) => codes ?? [],
                                ),
                            ),
                        )}
                        ...
                        retryResult={
                            lastRetryResult
                                ? {
                                      ...
                                      failedScopeSummary:
                                          lastRetryResult.failedByScope
                                              .map(
                                                  (entry) =>
                                                      `${scopeLabel(entry.scope)} ${entry.count}건`,
                                              )
                                              .join(", "),
                                  }
                                : null
                ...
                <Suspense fallback={null}>
                    <AutoApplyRecurringModal
                        ...
                        onConfirm={async () => {
                            ...
                            showToast(
                                `정기 항목 ${autoApplyPending.length}건 등록: ${formatRecurringToastSummary(autoApplyPending)}`,
                            );
                        }}
                ...
                >
                    <div className="w-full sm:max-w-sm bg-[#0D0F14] border border-white/10 rounded-t-3xl sm:rounded-3xl p-5 space-y-4">
                        ...
                        <button
                            ...
                            disabled={authBusy}
                            className="w-full py-2.5 rounded-xl bg-[#09f]/80 disabled:opacity-50 text-white text-sm font-bold flex items-center justify-center gap-2"
                        >
                            {authBusy ? (
            ...
            {confirmModal && (
                ...
                >
                    <div
                        className="bg-[#1C1C1E] rounded-3xl p-6 mx-4 max-w-xs w-full border border-white/8"
                        onClick={(e) => e.stopPropagation()}
                    >
                        <p className="text-white font-semibold text-center text-[15px] mb-6">
                            {confirmModal.message}
                        </p>
                        <div className="flex gap-2">
                            <button
                                onClick={() =>
                                    dispatchUI({type: "CLOSE_CONFIRM"})
                                }
